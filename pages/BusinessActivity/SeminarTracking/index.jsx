...
export default function SeminarTracking() {
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      if (!jsonObject.idSeminarTracking) {
        addPipeline = await fetchWithInterceptor(
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const rowDelete = await fetchWithInterceptor(
            "/api/proxy?api=deleteSeminarTracking",
            false,
            {},
            {
              method: "DELETE",
  ...
  const handleEditModal = async (id) => {
    const getSeminarData = await fetchWithInterceptor(
      `/api/proxy?api=getSeminarTrackingByID?idSeminarTracking=${id}`,
      false
    );
    const getSemeniarRes = getSeminarData;
