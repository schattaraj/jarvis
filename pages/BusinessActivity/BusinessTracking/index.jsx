...
export default function BusinessTracking() {
  const [columnNames, setColumnNames] = useState([
    ...
    {
      data: "totalAssetManagement",
      column_name: "Total Assets Under Management",
    },
    ...
    {
      data: "outBoundId",
      column_name: "Id of Outbound prospect/client engagement requests sent",
    },
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const rowDelete = await fetchWithInterceptor(
            "/api/proxy?api=deleteBusinessTracking",
            false,
            {},
            {
              method: "DELETE",
  ...
  const updateBusinessTracking = async (e) => {
    ...
    try {
      ...
      const response = await fetchWithInterceptor(
        "/api/proxy?api=editBusinessTracking",
        false,
        {},
        {
          method: "POST",
  ...
  const fetchAdvisorWise = async (e) => {
    ...
    try {
      ...
      const fetchAdvisor = await fetchWithInterceptor(
        url + "&_=1720608071572=",
        false
      );
      const fetchAdvisorRes = fetchAdvisor;
    ...
    async function run() {
      if (tableData.length > 0) {
        ...
        const totalAssetBroughtIn = items.reduce((acc, item) => {
          return (
            acc +
            (Number(item.assetBroughtIn) ? Number(item.assetBroughtIn) : 0)
          );
        }, 0);
        ...
        const totalAssetManagement = items.reduce((acc, item) => {
          return (
            acc +
            (Number(item.totalAssetManagement)
              ? Number(item.totalAssetManagement)
              : 0)
          );
        }, 0);
  ...
  return (
    <>
      <div className="main-panel">
        <div className="content-wrapper">
          ...
          <div className="d-md-flex justify-content-between">
            <div className="dt-buttons">
              ...
              <button
                ...
                onClick={handleOpen}
              >
                <span>Add Business Tracking</span>
              </button>
              <button
                className="dt-button buttons-html5 btn-primary mb-3"
                type="button"
              ...
              >
                ...
                <span className="d-none">Select Date Range</span>
              </button>
            </div>
          ...
          {dates?.startdate && dates?.enddate && (
            ...
            >
              From <b>{dates?.startdate}</b> to <b>{dates?.enddate}</b>
            </span>
          )}
                ...
                <tr>
                  {columnNames.map((columnName, index) => (
                    <th
                      ...
                      className={
                        columnName.data === "idBusinessTracking"
                          ? "sticky-action"
                          : columnName.data == "advisorName"
                          ? "sticky-left"
                          : ""
                      }
                    >
                ...
                {filterData.map((rowData, rowIndex) => (
                  <tr key={rowIndex} style={{ overflowWrap: "break-word" }}>
                    {columnNames.map((columnName, colIndex) => {
                      ...
                      if (
                        columnName.data == "assetBroughtIn" ||
                        columnName.data == "totalAssetManagement"
                      ) {
                        ...
                        return (
                          <td key={colIndex} className="sticky-action">
                            <button
                              ...
                              onClick={() => {
                                editBusinessTracking(rowData[columnName.data]);
                              }}
                            >
          ...
          >
            ...
            <div className="row">
              <div className="col-md-6">
                ...
                >
                  <Form.Label>
                    No. of Outbound prospect/client engagement requests sent
                  </Form.Label>
                  <Form.Control
                    type="text"
                    name="outBoundId"
            ...
            <div className="row">
              <div className="col-md-6">
                ...
                >
                  <Form.Label>
                    Which client was assets brought in from?
                  </Form.Label>
                    ...
                    {reviewMeetingWhom.map((asset, index) => (
                      <div className="asset-item" key={index}>
                        ...
                        <button
                          ...
                          aria-label="Close"
                          onClick={() =>
                            removeAsset(asset, "reviewMeetingWhom")
                          }
                        >
                          &times;
                        </button>
      ...
      <Modal show={editModal} onHide={closeEditModal}>
        ...
        <Modal.Body>
          <Form
            onSubmit={updateBusinessTracking}
            autoComplete={"true"}
            method="POST"
            encType="multipart/form-data"
          >
            <input
              type="hidden"
              name="idBusinessTracking"
              value={editData?.idBusinessTracking}
            />
            <div className="row">
              <div className="col-md-6">
                <Form.Group
                ...
                >
                  ...
                  <Form.Select
                    aria-label="Default select example"
                    name="advisorName"
                    defaultValue={editData?.advisorName}
                    required
                  >
                    <option value={""}>--Select--</option>
                    <option value="Noland">Noland</option>
                ...
                >
                  ...
                  <Form.Control
                    type="text"
                    name="assetBroughtFrom"
                    value={editData?.assetBroughtFrom}
                    onChange={handleEditData}
                    required
                  />
                </Form.Group>
              ...
              <div className="col-md-6">
                ...
                >
                  ...
                  <Form.Control
                    ...
                    name="totalAssetManagement"
                    value={
                      editData?.totalAssetManagement &&
                      Number(editData?.totalAssetManagement.replace(/,/g, ""))
                    }
                    onChange={handleEditData}
                    required
                  />
                ...
                >
                  ...
                  <Form.Control
                    type="text"
                    name="firstMeetingWhom"
                    value={editData?.firstMeetingWhom}
                    onChange={handleEditData}
                    required
                  />
                </Form.Group>
                ...
                >
                  ...
                  <Form.Control
                    type="text"
                    name="secondMeetingWhom"
                    value={editData?.secondMeetingWhom}
                    onChange={handleEditData}
                  />
                </Form.Group>
                <Form.Group
                ...
                >
                  ...
                  <Form.Control
                    type="text"
                    name="reviewMeetingWhom"
                    value={editData?.reviewMeetingWhom}
                    onChange={handleEditData}
                    required
                  />
                </Form.Group>
        ...
        <Modal.Body className="mw-100">
          <Form onSubmit={fetchAdvisorWise}>
            <div className="row">
              <div className="col-md-6">
                <Form.Group className="mb-3" controlId="advisorName">
                  ...
                  <input
                    ...
                    value={
                      advisorName &&
                      advisorName.map((item) => item.value).join(",")
                    }
                  />
                </Form.Group>
              </div>
              <div className="col-md-6">
                <Form.Group className="mb-3" controlId="timeFrame">
                  ...
                  <Form.Select
                    name="timeFrame"
                    defaultValue={editData?.timeFrame}
            ...
            <div className="table-responsive my-4">
              {advisorData && <StringToHTML htmlString={advisorData} />}
            </div>
            <div className="d-flex justify-content-end">
              <button
