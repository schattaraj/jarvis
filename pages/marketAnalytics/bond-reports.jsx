  ...
  const fetchColumnNames = async () => {
    ...
    try {
      const columnApiRes = await fetchWithInterceptor(
        "/api/proxy?api=getColumns?metaDataName=Debt_Report_Matrices&_=1705582308870"
      );
  ...
  const handleReportData = async (name) => {
    ...
    try {
      const fetchReportRes = await fetchWithInterceptor(
        "/api/proxy?api=getTickerReportsByTickerName?tickerName=" + name
      );
  ...
  const go = async () => {
    if (selectedTicker) {
      ...
      try {
        ...
        // const getBondsRes = await getBonds.json();
        const Debt_Report_MatricesRes = getBondsRes.filter(
          (item) => item.element1 === selectedTicker
        );
        setTableData(Debt_Report_MatricesRes);
                ...
                {reportData.map((item, index) => {
                  return (
                    <tr key={"report" + index}>
                      ...
                      <td>
                        <p style={{ maxWidth: "400px" }} className="text-wrap">
                          {item?.description}
                        </p>
                      </td>
