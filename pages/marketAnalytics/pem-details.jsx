  ...
  const fetchColumnNames = async () => {
    try {
      const columnApiRes = await fetchWithInterceptor(
        "/api/proxy?api=getColumns?metaDataName=PEM&_=1706681174127"
      );
  ...
  const fetchData = async () => {
    ...
    try {
      const getBondsRes = await fetchWithInterceptor(
        "/api/proxy?api=getImportsData?metaDataName=PEM&_=1706681174128"
      );
                  ...
                  <tr key={rowIndex} style={{ overflowWrap: "break-word" }}>
                    {columnNames.map((columnName, colIndex) => {
                      ...
                      if (columnName.elementInternalName === "element3") {
                        ...
                        columnName.elementInternalName === "element80"
                      ) {
                        content = (
                          Number.parseFloat(
                            rowData[columnName.elementInternalName]
