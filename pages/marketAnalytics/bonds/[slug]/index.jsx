        ...
        <>
            <div className="main-panel">
                <div className="content-wrapper">
                                        ...
                                        return <tr key={"tr" + index}>
                                            {
                                                columnNames.map((inner, keyid) => {
                                                    return <td key={"keyid" + keyid}>{inner['elementInternalName'] == 'lastUpdatedAt' ? formatDate(item['lastUpdatedAt']) : typeof (item[inner['elementInternalName']]) == "string" ? parse(item[inner['elementInternalName']]) : item[inner['elementInternalName']]}</td>
                                                })
                                            }
                                        </tr>
                                    })
                    ...
                    {chartData.length > 0 && <HighchartsReact highcharts={Highcharts} options={chartOption} />}
                    ...
                    <div className="table-responsive mb-4">
                        <table className="table border display no-footer dataTable" role="grid" aria-describedby="exampleStocksPair_info" id="my-table">
                            <thead>
                                <tr>
                                    {
                                        bondStockColumns.length > 0 && bondStockColumns.map((item, index) => {
                                            return <th key={index} onClick={() => handleSort(item?.elementInternalName)}>{item?.elementName} {getSortIcon(item?.elementInternalName, sortConfig)}</th>
                                        })
                                    }
                                </tr>
                            ...
                            <tbody>
                                ...
                                <tr><td colSpan={bondStockColumns.length}>No Data Available</td></tr>
                                }
                            </tbody>
                        </table>
                    </div>
