...
export default function SaveSecurities() {
  ...
  const [sortAllBond, setSortAllBond] = useState({
    key: null,
    direction: null,
  });
  const [sortFavBond, setSortFavBond] = useState({
    key: null,
    direction: null,
  });
  ...
  const fetchAllStockData = async () => {
    ...
    try {
      const findAllStocksRes = await fetchWithInterceptor(
        "/api/proxy?api=findAllStocks"
      );
  ...
  const fetchAllFavoriteStockData = async () => {
    context.setLoaderState(true);
    try {
      const findAllFavoriteStocksRes = await fetchWithInterceptor(
        "/api/proxy?api=findAllFavoriteStocks?userid=2"
      );
  ...
  const fetchAllBonds = async () => {
    context.setLoaderState(true);
    try {
      const findAllBondsRes = await fetch("/api/proxy?api=findAllBonds");
      // const findAllBondsRes = await findAllBonds.json()
      setAllBonds(findAllBondsRes);
      setAllBondsFiltered(
        SliceData(bondCurrentPage, allBondLimit, findAllBondsRes)
      );
    } catch (error) {
      console.log("Error", error);
    }
  ...
  const fetchAllFavoriteBonds = async () => {
    context.setLoaderState(true);
    try {
      const findAllFavoriteBondsRes = await fetchWithInterceptor(
        "/api/proxy?api=findAllFavoriteBonds?userid=2"
      );
      // const findAllFavoriteBondsRes = await findAllFavoriteBonds.json()
      setAllFavoriteBonds(findAllFavoriteBondsRes);
      setAllFavoriteBondsFiltered(
        sliceData(allFavoriteBonds, favBondCurrentPage)
      );
    } catch (error) {
      console.log("Error", error);
    }
  ...
  useEffect(() => {
    allStockData.length > 0 &&
      setStockDataFiltered(sliceData(allStockData, stockCurrentPage));
  }, [allStockData, stockCurrentPage]);
  useEffect(() => {
    allFavoriteStockData.length > 0 &&
      setFavoriteStockDataFiltered(
        sliceData(allFavoriteStockData, favStockCurrentPage)
      );
                ...
                <div className="col-md-6">
                  <div className="card p-3">
                    ...
                    <Pagination
                      ...
                      totalItems={allStockData}
                      limit={limit}
                      setCurrentPage={setStockCurrentPage}
                      handlePage={handleAllStockPage}
                    />
                  </div>
                </div>
                <div className="col-md-6">
                  <div className="card p-3">
                    ...
                    <div className="table-responsive">
                      <table className="table">
                        ...
                        <tbody>
                          ...
                          {allFavoriteStockDataFiltered.length == 0 && (
                            <tr>
                              <td colSpan={6} className="text-center">
                                No data available in the table
                              </td>
                    ...
                    <Pagination
                      currentPage={favStockCurrentPage}
                      totalItems={allFavoriteStockData}
                      limit={limit}
                      setCurrentPage={setFavStockCurrentPage}
                      handlePage={handleFavStockPage}
                    />
                  </div>
                </div>
              </div>
                  ...
                  <div className="card p-3">
                    <div className="d-flex justify-content-between">
                      ...
                      <div className="form-group d-flex flex-wrap flex-sm-nowrap align-items-center">
                        ...
                        <select
                          name="limit"
                          className="form-select w-auto mb-0 me-2"
                          style={{ fontSize: "14px" }}
                          onChange={changeBondLimit}
                          value={allBondLimit}
                        >
                      ...
                      <table className="table">
                        <thead>
                          <tr>
                            <th
                              onClick={() => {
                                handleAllBondSort("issuerName");
                              }}
                            >
                              Issuer Name{" "}
                              {getSortIcon("issuerName", sortAllBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleAllBondSort("type");
                              }}
                            >
                              Bond Type {getSortIcon("type", sortAllBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleAllBondSort("ytm");
                              }}
                            >
                              YTM {getSortIcon("ytm", sortAllBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleAllBondSort("coupon");
                              }}
                            >
                              Coupon {getSortIcon("coupon", sortAllBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleAllBondSort("maturity");
                              }}
                            >
                              Matuirity {getSortIcon("maturity", sortAllBond)}
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                    ...
                    <Pagination
                      ...
                      totalItems={allBonds}
                      limit={allBondLimit}
                      setCurrentPage={setBondCurrentPage}
                      handlePage={handleBondsPage}
                    />
                  </div>
                </div>
                <div className="col-md-6">
                  <div className="card p-3">
                      ...
                      <table className="table">
                        <thead>
                          <tr>
                            <th
                              onClick={() => {
                                handleFavBondSort("issuerName");
                              }}
                            >
                              Issuer Name{" "}
                              {getSortIcon("issuerName", sortFavBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleFavBondSort("type");
                              }}
                            >
                              Bond Type {getSortIcon("type", sortFavBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleFavBondSort("ytm");
                              }}
                            >
                              YTM {getSortIcon("ytm", sortFavBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleFavBondSort("coupon");
                              }}
                            >
                              Coupon {getSortIcon("coupon", sortFavBond)}
                            </th>
                            <th
                              onClick={() => {
                                handleFavBondSort("maturity");
                              }}
                            >
                              Matuirity {getSortIcon("maturity", sortFavBond)}
                            </th>
                            <th>Cusio No</th>
                          </tr>
                        </thead>
                        <tbody>
                          ...
                          {allFavoriteBondsFiltered.length == 0 && (
                            <tr>
                              <td colSpan={6} className="text-center">
                                No data available in the table
                              </td>
                    ...
                    <Pagination
                      currentPage={favBondCurrentPage}
                      totalItems={allFavoriteBonds}
                      limit={favBondLimit}
                      setCurrentPage={setFavBondCurrentPage}
                      handlePage={handleFavBondsPage}
                    />
                  </div>
                </div>
              </div>
